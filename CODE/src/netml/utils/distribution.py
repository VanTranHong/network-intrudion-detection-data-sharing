...
def distribute_dataframe_np(X,y,num_agents,imba,seq_select,rng,label_name='label'):
    list_of_dfs=[]
    assert len(X)>=num_agents
    if not imba:
        print('Maintaining ratio')
        ...
        if seq_select:
            print('Selecting for agents in order')
...
def distribute_dataframe(df,num_agents,imba,seq_select,label_name='label'):
